import {FeaturesLayoutView} from '../components/FeaturesLayoutView';

import "../features.scss";

export const FeaturesLayoutContainer = () => {

	const featuresList = [
		{
			title: "Patients first, always",
			text: `We know how stressful it can be when your pet needs
            meds. That’s why we offer free next-day delivery and
            even do same-day in NYC and LA. We got you.`,
			isLine: true,
		},
		{
			title: "Tailored care",
			text: `	We’ll work with you and your veterinarian to find the
            very best solution for your pet. With our custom
            compounded options, we can change the dosage, flavor and
            form. We even have a flavor guarantee with over 40
            options! 
            <p>
            P.S. Everyone seems to love the
            chicken-flavored chew treats!
            </p>`,
			isLine: true,
		},
		{
			title: "Support, 7 days a week",
			text: `	You’re not alone. Text, call, or email us 7 days a
            week—we’ll answer and do our best to sprinkle in a bit
            of sunshine. ☀️`,
			isLine: false,
		},
	];

	const featuresElements = featuresList.map((feature, i) => {
		return <FeaturesLayoutView key={i} item={feature}/>
	});

	return (
		<div className="features__list">
			{featuresElements}
		</div>
	);
};
